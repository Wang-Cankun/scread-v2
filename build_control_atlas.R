...
load_test_data <- function() {
  ...
  expr_file = "H-H-Prefrontal_cortex-BA6_8_9-Male.fst"
  data_id <- 'AD01301'
}
...
dir.create("expr", showWarnings = F)
dir.create("label", showWarnings = F)
...
exp <-
  as.matrix(GetAssayData(object = health.obj[['RNA']], slot = "data"))
#exp <- log1p(exp)
#exp[] <- normalize.quantiles(exp)
rownames(exp) <- toupper(rownames(exp))

keep_scina_cell_type <-
  as.numeric(which(!sapply(signatures, function(x) {
...
health.obj <-
  AddMetaData(health.obj, as.factor(results$cell_labels), col.name = 'scina_cell_type')
...
for (i in 1:length(levels(Idents(health.obj)))) {
  this_cluster_cells <-
    WhichCells(health.obj, idents = levels(Idents(health.obj))[i])
  ...
  if ('unknown' %in% levels(Idents(this_scina_obj))) {
    ...
    max_ct <-
      names(which.max(table(Idents(this_scina_obj))[1:scina_obj_ident_length]))
  } else {
...
health.obj <-
  AddMetaData(health.obj, as.factor(annotate_cell_type), col.name = 'predicted.id')
...
png(
  paste("png/", data_id, "_umap.png", sep = ""),
  width = 3000,
  height = 1500,
  res = 300
)
...
saveRDS(health.obj, file = paste0("rds/", data_id, '.rds'))
...
write.table(
  ...
  paste("expr/", data_id, "_expr.txt", sep = ""),
  row.names = F,
  sep = "\t",
  quote = FALSE
)
...
write.table(
  cell_label,
  paste("label/", data_id, "_cell_label_v2.txt", sep = ""),
  quote = F,
  row.names = F,
  sep = "\t"
)
...
#od <- order(names(table(cell_info)))
#barplot(table(cell_info)[od])
