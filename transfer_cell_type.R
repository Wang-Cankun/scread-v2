...
load_test_data <- function(){
  ...
  control_filename <- "AD00101.rds"
  disease_filename <- "H-AD-Prefrontal_cortex-BA6_8_9-Male_001.fst"
  disease_data_id <- "AD01303"
}
...
####### Load raw files
health.obj <- read_rds(paste0("./rds/",control_filename))
...
png(paste("./png/",disease_data_id,"_transfer_umap.png",sep = ""),width=4000, height=2000,res=300)
plot_grid(p1,p2)
dev.off()

Idents(dstage.obj) <- dstage.obj$predicted.id
saveRDS(dstage.obj, paste0("rds/", disease_data_id,".rds"))

exp_data <- GetAssayData(object = dstage.obj,slot = "counts")

write.table(data.frame("Gene"=rownames(exp_data),exp_data,check.names = F),paste("expr/", disease_data_id,"_expr.txt",sep = ""), row.names = F,sep="\t",quote=FALSE)
...
write.table(cell_label,paste("label/", disease_data_id,"_cell_label_v2.txt",sep = ""),quote = F,row.names = F,sep = "\t")
