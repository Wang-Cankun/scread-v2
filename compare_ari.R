...
labels <- list.files(pattern = "*_v2.txt")
...
ari <- vector()
for (i in ids) {
  this_v1_label <- read_tsv(paste0(i, "_cell_label.txt"))
  this_v2_label <- read_tsv(paste0(i, "_cell_label_v2.txt"))
...
## AD00201 with error IN neurons

### Compare umap
obj <- readRDS(paste0("/fs/scratch/PCON0022/ad/input/fst/rds/AD00201.rds"))
this_label <- as.data.frame(this_label)
rownames(this_label) <- this_label$cell_name
this_label <- this_label[colnames(obj), ]

level1 <- as.factor(as.character(obj$predicted.id))
level1 <- relevel(level1, "Inhibitory neurons")
level1 <- factor(level1, levels=rev(levels(level1)))
Idents(obj) <- level1
...
level2 <- as.factor(as.character(obj$v1))
#level2 <- relevel(level2, "Inhibitory neurons")
level2 <- factor(level2, levels=rev(levels(level2)))
Idents(obj) <- level2
...
marker <- 
FeaturePlot(obj, features = c("GAD1","GAD2"), pt.size = 1)

FeaturePlot(obj, features = signatures$Excitatory.neurons, pt.size = 1)
